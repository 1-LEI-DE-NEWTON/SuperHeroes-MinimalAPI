...
app.MapGet("/", () => "Bem-vindo ao banco de dados de super heróis!");
...
app.MapGet("/superhero/{id}", async (DataContext context, int id) =>
    ...
    Results.Ok(hero) :
    Results.NotFound("Perdão, super herói não encontrado"));
...
app.MapPut("/superhero/{id}", async (DataContext context, int id, SuperHero hero) =>
{
    ...
    if (dbHero is null)
        return Results.NotFound("Perdão, super herói não encontrado");
